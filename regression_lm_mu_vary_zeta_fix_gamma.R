...
# This script uses lm for mu in first stage (and uses s= lambda.min for pi)
...
for (zeta in seq(0, 50, 5)) {
  ...
  results <- foreach(sim_num = 1:n_sim) %dopar% {
    ...
    mu_lasso <- cv.glmnet(x[((s == 2) & (a == 0)), ], y0[((s == 2) & (a == 0))])# if s= 0 fails, try s= mu_lasso$lambda[100]
    muhat <- as.numeric(predict(mu_lasso, newx = x, s = 0))
