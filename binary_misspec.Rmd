...
results <- foreach(sim_num = 1:n_sim) %dopar% {
  ...
  mu0 <- sigmoid(as.numeric(z %*% rep(c(1, 0), c(zeta, q - zeta)) +
    v_second_order %*% c(rep(c(1, -1), gamma/4), rep(0,p/2 - gamma/2))+
       v_first_order %*% rep(c(1, 0), c(gamma/2, p/2 - gamma/2)))/sqrt(beta*0.01))
  ...
  bcrt_muest_lasso <- cv.glmnet(v[s == 3, ], bc_rct_muest[s == 3])
  bcr_muest <- predict(bcrt_muest_lasso, newx = v, s = "lambda.min")
