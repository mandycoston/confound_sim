...
for (alpha_z in seq(0, 40, 5)) {
  ...
  results <- foreach(sim_num = 1:n_sim) %dopar% {
    ...
    prop <- sigmoid(as.numeric(x %*% rep(c(1, 0, 1, 0), c(alpha_z, q - alpha_z, alpha_v, p - alpha_v))) / sqrt(alpha)) # divide by sqrt(4*alpha) for not prop widen
    a <- rbinom(n, 1, prop)
    y0 <- rbinom(n, 1, mu0)
  ...
  saveRDS(tibble(
    ...
    "alpha_z" = alpha_z,
    "alpha" = alpha
  ), glue::glue(results_folder, "alpha_z_{alpha_z}", "parameters.Rds"))
